...
class ApiKeyLifecycleTest < ActionDispatch::IntegrationTest
  ...
  test "complete API key lifecycle from creation to revocation" do
    ...
    created_keys.each do |key_data|
      ...
      json = JSON.parse(response.body)
      assert json["success"]
      assert_equal "Test with #{key_data[:name]}", json["set"]["exercise"]
    end
    ...
    active_keys = api_keys.select { |key| key["active"] }
    inactive_keys = api_keys.select { |key| !key["active"] }
    
    assert_equal 3, active_keys.length # 2 remaining + 1 from setup
    assert_equal 2, inactive_keys.length
    ...
    assert_equal 4, api_keys.length # One key was deleted
  ...
  test "API key security and validation workflow" do
    ...
    5.times do |i|
      ...
      if response.status == 201
        json = JSON.parse(response.body)
        rapid_keys << json["api_key"]["raw_key"]
      end
    end
  ...
  test "API key usage tracking and audit" do
    ...
    # Verify audit logs exist for tool usage
    assert McpAuditLog.count > 0
